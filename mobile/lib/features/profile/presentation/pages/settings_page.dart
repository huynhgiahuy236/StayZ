            ...
            children: [
              ...
              ...AppLanguage.values.map((lang) {
                return _LanguageOption(
                  flag: AppLocale.flagFor(lang),
                  label: AppLocale.labelFor(lang),
                  selected: AppLocale.instance.language == lang,
                  onTap: () async {
                    await AppLocale.instance.setLanguage(lang);
...
class _ProfileStatsState extends State<_ProfileStats> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<int>>(
      future: _future,
      builder: (context, snapshot) {
        final trips = snapshot.data?[0] ?? 0;
        final saved = snapshot.data?[1] ?? 0;
        return Row(
          children: [
            Expanded(
              child: _Stat(
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<StayzUser?>(
      ...
      builder: (context, snapshot) {
        ...
        if (user == null) {
          return Container(
            padding: EdgeInsets.all(20 * widget.responsive.scale),
            decoration: BoxDecoration(
              color: isDark
            ...
            child: Row(
              children: [
                CircleAvatar(
                  radius: 32 * widget.responsive.scale,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        tr('Chào mừng đến StayZ', 'Welcome to StayZ'),
                        style: TextStyle(
                          fontSize: 18 * widget.responsive.scale,
                          fontWeight: FontWeight.w900,
                        ),
                      ),
                      ...
                      Text(
                        tr(
                          'Đăng nhập để xem lịch sử & quản lý tài khoản.',
                          'Sign in to view history & manage account.',
                        ),
                        style: TextStyle(
                          fontSize: 12 * widget.responsive.scale,
                          color: Theme.of(context).colorScheme.secondary,
                        ),
                      ),
                      ...
                      ElevatedButton(
                        ...
                        child: Text(
                          tr('Đăng nhập / Đăng ký', 'Sign in / Sign up'),
